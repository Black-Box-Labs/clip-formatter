...
def test_date_with_return_original(formatter):
    ...
    expected = pd.DataFrame({
        ...
        "org_date": [datetime.datetime(2024, 3, 22), datetime.datetime(2020, 1, 1), datetime.datetime(2019, 8, 15), np.nan]
    })
    result = formatter.date(raw_dates, return_original=True)
    pd.testing.assert_frame_equal(result, expected)

def test_date_mixed_formats(formatter):
    raw_dates = pd.Series(["2024-03-22", "March 22, 2024", "22nd Mar 2024", "2024/03/22"])
